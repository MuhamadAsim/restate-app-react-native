...
const AddDocumentModal = ({ visible, onClose, onDocumentAdded, prefilledFile }) => {
 ...
 // Add this useEffect near the top after your state declarations
useEffect(() => {
  if (prefilledFile && visible) {
    console.log("📎 [PREFILLED FILE FROM QR]", prefilledFile);
    
    // Set the downloaded file
    setSelectedFile({
      uri: prefilledFile.uri,
      name: prefilledFile.name,
      size: prefilledFile.size,
      mimeType: prefilledFile.mimeType,
    });
    
    // Auto-generate a clean document name
    const fileName = prefilledFile.name;
    // Remove "Scanned_timestamp." prefix for cleaner name
    let cleanName = fileName.replace(/^Scanned_\d+\./, "");
    cleanName = cleanName.substring(0, cleanName.lastIndexOf('.')) || cleanName;
    cleanName = cleanName || "QR Scanned Document";
    
    setDocumentName(cleanName);
    
    console.log("✅ [AUTO-FILLED] Name:", cleanName);
  }
}, [prefilledFile, visible]);
